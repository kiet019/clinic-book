...
function ChatDoctor() {
    const { data: doctorList, isFetching: doctorListFetching } = useGetListDoctor();
    const { data: doctorData, isFetching: doctorDataFetching } = useGetDoctorProfile();
    ...
    useEffect(() => {
        ...
        if (doctorData && doctorData.doctorName) {
            const messagesRef = ref(db, `chatRooms/${doctorData.doctorName}/messages`);
    ...
    const sendMessage = () => {
        if (doctorData && doctorData.doctorName && newMessage.trim() !== '') {
            ...
            .then(() => {
                setMessages([...messages, { sender: doctorData.doctorName, text: newMessage, timestamp: Date.now() }]);
                setNewMessage('');
            })
            .catch(error => {
    ...
    // Loading state
    if (doctorListFetching || doctorDataFetching || !doctorData || !doctorData.doctorName) {
    ...
    <div className="media-body">
      <div>
        <div className="user-name">Bác sĩ {doctorData.doctorName}</div>
                                    ...
                                    <div className="media">
                                        <div className="media-img-wrap">
                                            <div className="avatar avatar-online">
                                                <img src={doctorData.imageUrl || "assets/img/doctors/doctor-thumb-02.jpg"} alt="User Image" className="avatar-img rounded-circle" />
                                            </div>
                                        </div>
                                        <div className="media-body">
                                            <div className="user-name">Bác sĩ {doctorData.doctorName}</div>
